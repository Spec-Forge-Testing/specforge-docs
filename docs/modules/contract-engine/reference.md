...
- **Producer-only sections ride along verbatim.** `risk`, `attack`, `transitions`,
  `semantic_properties` and `access` are not schemas and the OpenAPI base cannot carry
...
#     plus any "risk" / "attack" / "transitions" / "semantic_properties" /
#     "access" the LLM contract carried, copied as is
```
