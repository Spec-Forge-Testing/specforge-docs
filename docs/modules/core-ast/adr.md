...
| | Stage | Decision |
| --- | --- | --- |
| [ADR-001](#adr-001) | locator | Build output and dependency directories are excluded from the scan |
| [ADR-002](#adr-002) | locator | The repository scan is cached per repository, and what is cached is immutable |
| [ADR-003](#adr-003) | locator | The scan cache is invalidated explicitly, never automatically |
| [ADR-004](#adr-004) | package | Constants live with the stage that uses them |
| [ADR-005](#adr-005) | locator | The route vocabulary is deliberately narrow |
| [ADR-006](#adr-006) | locator | An extension inherits another's rules only within the same language |
| [ADR-007](#adr-007) | locator | A wildcard-only route form can name a handler, never choose a file |
| [ADR-008](#adr-008) | locator | Every pattern is scoped to one language |
| [ADR-009](#adr-009) | locator | Route templates are tried one at a time, never merged |
| [ADR-010](#adr-010) | locator | Following a router mount is two hops, and returns a scope |
| [ADR-011](#adr-011) | locator | Routes generated by a resource registration are located by convention |
| [ADR-012](#adr-012) | locator | An ambiguous strategy does not abort the cascade |
| [ADR-013](#adr-013) | locator | Naming a handler uses method-level declarations only |
| [ADR-014](#adr-014) | locator | The keyword-less heuristic needs two guards |
| [ADR-015](#adr-015) | locator | A handler named inside the declaration wins, if this file defines it |
| [ADR-016](#adr-016) | locator | Following a reference out of the routes file, and the single-definition rule |
| [ADR-017](#adr-017) | locator | With parameters in the path, the declaration that writes them wins |
| [ADR-018](#adr-018) | ast_builder | Five grammars ship with the package, seven are an optional extra |
| [ADR-019](#adr-019) | ast_builder | Languages that share a grammar's shape share its tag query |
| [ADR-020](#adr-020) | ast_builder | The syntax gate only rejects an unusable tree |
| [ADR-021](#adr-021) | ast_builder | An `operationId` only names the handler if it names something callable |
| [ADR-022](#adr-022) | ast_builder | The tag queries and `[class_keywords]` are checked against each other |
| [ADR-023](#adr-023) | extractor | The function is cut by byte offsets, decorators included |
| [ADR-024](#adr-024) | extractor | A handler name can be a dotted path, resolved segment by segment |
| [ADR-025](#adr-025) | tracer | Call detection is table-driven, and walks the tree by hand |
| [ADR-026](#adr-026) | tracer | A call is reported qualified, and the tracer decides what to do with it |
| [ADR-027](#adr-027) | tracer | The snippet is reparsed on its own |
| [ADR-028](#adr-028) | tracer | Unresolved and external are different failures |
| [ADR-029](#adr-029) | tracer | Where the package is the directory, siblings are swept |
| [ADR-030](#adr-030) | tracer | The non-traceable table is per language and includes what is never imported |
| [ADR-031](#adr-031) | tracer | Imports are split three ways, not two |
| [ADR-032](#adr-032) | tracer | A file never imports itself |
| [ADR-033](#adr-033) | tracer | A grouped import becomes one name per symbol |
| [ADR-034](#adr-034) | tracer | Import parsing reads grammar fields, never positions |
| [ADR-035](#adr-035) | tracer | Nine resolvers, one template, two hooks |
| [ADR-036](#adr-036) | tracer | An import can resolve to a directory, or to an index file |
| [ADR-037](#adr-037) | quality | A critical unresolved call forces fallback, whatever the ratio |
| [ADR-038](#adr-038) | quality | No expected calls means a ratio of 1.0 |
| [ADR-039](#adr-039) | quality | The fallback bundle has a hard budget and a fixed priority |
| [ADR-040](#adr-040) | packager | The code travels in CDATA, and the renderer touches no disk |
| [ADR-041](#adr-041) | packager | Token count uses tiktoken when it is installed, a heuristic when it is not |
| [ADR-042](#adr-042) | api | The handler is identified in three steps, and the file can change on the way |
| [ADR-043](#adr-043) | api | A batch isolates endpoint failures, but not run failures |
| [ADR-044](#adr-044) | models | Pydantic on the boundaries, dataclasses inside |
| [ADR-045](#adr-045) | exceptions | An exception says what happened, not what to do about it |
...
This is the same idea as [ADR-006](#adr-006) —`.tsx` inheriting `.ts`'s
...
([ADR-022](#adr-022)), and the failure was silent: the dependency simply
...
twelve languages detected zero calls ([ADR-025](#adr-025)), every endpoint
