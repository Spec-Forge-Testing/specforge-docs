...
## ADR-037 — A critical unresolved call forces fallback, whatever the ratio
...
## ADR-038 — No expected calls means a ratio of 1.0
...
twelve languages detected zero calls ([ADR-025](adr.md#adr-025)), every endpoint
...
## ADR-039 — The fallback bundle has a hard budget and a fixed priority
...
## ADR-040 — The code travels in CDATA, and the renderer touches no disk
...
## ADR-041 — Token count uses tiktoken when it is installed, a heuristic when it is not
...
## ADR-042 — The handler is identified in three steps, and the file can change on the way
...
1. **The locator already knows.** A route generated by a resource registration
   ...
   handler name ([ADR-011](adr.md#adr-011)).
2. **The `operationId` names something callable in this file.** Not any
   definition — a type with the same name does not count
   ([ADR-021](adr.md#adr-021)).
3. **Infer it** from the route declaration ([ADR-013](adr.md#adr-013)).
...
## ADR-043 — A batch isolates endpoint failures, but not run failures
